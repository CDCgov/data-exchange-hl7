    ...
    fun branch(
        ...
        try {
            mappedMessages.forEachIndexed { i: Int, mappedMessage: Map<String, Any> ->
                try {
                    val id = mappedMessage["id"] as String
                    ...
                    if (processStatus == "SUCCESS") {
                        logger.info("DEX::To OK eventhub [${i + 1}] id: $id, processName=$processName, status=$processStatus")
                        outOkList.add(objectMapper.writeValueAsString(mappedMessage))
                    } else {
                        logger.info("DEX::To ERR eventhub [${i + 1}] id: $id, processName=$processName, status=$processStatus")
