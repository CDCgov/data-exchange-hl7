...
class Function {
    ...
    private fun sendMessageAndUpdateEventReport2(payload: DexEventPayload2, eventReport: ReceiverEventReport) {
        try {
            logger.info("DEX::Processed messageUUID: ${payload.messageUUID}")
            fnConfig.evHubSenderOut.send(gson.toJson(payload))
        } catch (e: Exception) {
    ...
    private fun getMessageInfo2(metaDataMap: Map<String, String?>, message: String): DexMessageInfo2 {
        val eventCode = extractValue(message, EVENT_CODE_PATH)
        val localRecordID = extractValue(message, LOCAL_RECORD_ID_PATH)
        val messageType = metaDataMap["message_type"]
        ...
        if (messageType == HL7MessageType.ELR.name) {
            val route = metaDataMap["route"]?.normalize()
            val reportingJurisdiction = metaDataMap["reporting_jurisdiction"]
            return DexMessageInfo2(eventCode, route, null, reportingJurisdiction, HL7MessageType2.ELR, localRecordID)
        }
        ...
        return DexMessageInfo2(
            eventCode = eventCode,
            route = fnConfig.eventCodes[eventCode]?.get("category") ?: "",
            mmgKeyList = null,
            jurisdictionCode = jurisdictionCode,
            type = HL7MessageType2.CASE,
            localRecordID = localRecordID
        )
    }
    ...
    private fun buildMetadata2(
        ...
        provenance: Provenance,
        errorMessage: String? = null
    ): Pair<DexMetadata2, SummaryInfo> {
        val processMD = ReceiverProcessMetadata2(status, eventGridMD)
        processMD.startProcessTime = startTime
        processMD.endProcessTime = Date().toIsoString()
        var summary = SummaryInfo("RECEIVED")
        if (status == STATUS_ERROR) {
        ...
        return DexMetadata2(provenance, listOf(processMD)) to summary
    }
    ...
    private fun preparePayload2(
        messageContent: ArrayList<String>,
        messageInfo: DexMessageInfo2,
        metadata: DexMetadata2,
        summary: SummaryInfo,
        metaDataMap: Map<String, String?>
    ): DexEventPayload2 {

        return DexEventPayload2(
            //      uploadID = uploadID,
            //       destinationID = metaDataMap["meta_destination_id"],
            //       destinationEvent = metaDataMap["meta_ext_event"],
            messageInfo = messageInfo,
            ...
            //for ((eventIndex, eventMessage) in events.withIndex()) {
               ...
               try {
                   ...
                   // val event = eventArr
                    ...
                    // Pick up blob metadata
                    val blobName = event.evGridData.url.substringAfter("/${fnConfig.blobIngestContName}/")
                   ...
                   // filter out known/required metadata and store the rest in
                    ...
                    if (!isValidMessage) {
                        // required Metadata is missing -- send to error queue
                        val errorMessage = "Message missing required metadata."
                        val (metadata, summary) = buildMetadata2(
                            STATUS_ERROR,
                            "1",
                            startTime,
                            provenance,
                        ...
                        // send empty array as message content when content is invalid
                        msgEvent = preparePayload2(
                            arrayListOf(),
                            DexMessageInfo2(null, null, null, null, HL7MessageType2.valueOf(messageType)),
                            metadata,
                            summary,
                            metaDataMap
                        ).apply {
                            addErrorToReport(eventReport, errorMessage, this.messageUUID)
                            sendMessageAndUpdateEventReport2(this, eventReport)
                        }
                                ...
                                if (lineClean.startsWith("FHS") ||
                                    ...
                                    if (lineClean.startsWith("MSH")) {
                                        ...
                                        if (mshCount > 1) {
                                            ...
                                            provenance.messageHash = currentLinesArr.joinToString("\n").hashMD5()
                                            val messageInfo =
                                                getMessageInfo2(metaDataMap, currentLinesArr.joinToString("\n"))
                        ...
                        msgEvent = if (mshCount > 0) {
                            ...
                            val messageInfo = getMessageInfo2(metaDataMap, currentLinesArr.joinToString("\n"))
                            logger.info("message info --> ${gson.toJson(messageInfo)}")
                            preparePayload2(
                                currentLinesArr, messageInfo, metadata, summary, metaDataMap
                            ).apply {
                            ...
                            // send empty array as message content when content is invalid
                            preparePayload2(
                                arrayListOf(), DexMessageInfo2(
                                    null, null, null, null,
                                    HL7MessageType2.valueOf(messageType)
                                ),
                                metadata, summary, metaDataMap
                            ).apply {
                                addErrorToReport(
